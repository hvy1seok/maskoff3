...
class BasicClassifier(BasicModel):
    ...
    def _step(self, batch: dict, batch_idx: int, state: str, step: int):
        """Basic step for training and validation"""
        target = batch['target']
...
class BasicRegression(BasicModel):
    ...
    def _step(self, batch: dict, batch_idx: int, state: str, step: int):
        ...
        # Run Model 
        pred = self(source) # [B, num_classes]

        # Target 변환: one-hot [B, 3] -> class index [B]
        target_for_metric = target
        if len(target.shape) > 1 and target.shape[1] > 1:  # [B, 3] 형태인 경우
            target_for_metric = torch.argmax(target, dim=1)  # one-hot -> class index for metric
            # CORN loss를 위해서는 [B, 1] 형태로 유지
            target = target_for_metric.unsqueeze(-1)  # [B] -> [B, 1]
        ...
        # pred_labels는 [B, 1] 형태, target_for_metric은 [B] 형태
        if len(pred_labels.shape) > 1 and pred_labels.shape[1] == 1:
            pred_labels = pred_labels.squeeze(-1)  # [B, 1] -> [B]

        with torch.no_grad():
            # Aggregate here to compute for entire set later 
            self.mae[state+"_"].update(pred_labels, target_for_metric)
